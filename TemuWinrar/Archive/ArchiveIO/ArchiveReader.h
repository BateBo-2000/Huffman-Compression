...
private:
    ...
    static void readMasterOffsets(const std::string& archivePath,std::vector<uint64_t>& offsetsOut);

    static void readLocalHeaderAndName(std::ifstream& in, uint64_t offset,arch::LocalHeaderFixed& hOut,std::string& nameOut);

    static void readPayload(std::ifstream& in, const arch::LocalHeaderFixed& h, std::vector<uint8_t>& payloadOut);
