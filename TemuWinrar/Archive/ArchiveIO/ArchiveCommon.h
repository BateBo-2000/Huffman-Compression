...
namespace arch {
    ...
    enum : uint8_t {
        FLAG_IS_DIR = 1 << 0,   // directory entry
    };
...
#pragma pack(push,1)   //https://learn.microsoft.com/en-us/cpp/preprocessor/pack?view=msvc-170
    ...
    struct LocalHeaderFixed
    {
        ...
        uint8_t  hash[HASH_SIZE];  // hash of payload (compressed blob)
        uint32_t nameLen;          // bytes of name after header
    };
    //after that name bytes

    // At the end - list of offsets to LocalHeaderFixed (Fixed size)
    struct MasterHeaderFixed
    {
        uint32_t magic;            // MAGIC_MHDR
        uint64_t count;            // number of entries
         
    };// After this is a list of entries each points to a LocalHeaderFixed
